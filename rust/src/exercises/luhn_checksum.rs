...
pub fn is_valid(code: &str) -> bool {
  let cleaned = code.split(" ").collect::<Vec<&str>>().join("");

  // Non-numeric strings and strings of length 1 or less are not valid
  if !cleaned.parse::<u64>().is_ok() || cleaned.len() < 2 {
    return false;
  }

  // Luhn: Double every second digit, starting from the right
  // If the result > 9, subtract 9 from result
  let sum = cleaned
    .chars()
    .rev()
    .enumerate()
  ...
  // If sum of all digits is divisible by 10, number is valid
  return sum % 10 == 0;
}
