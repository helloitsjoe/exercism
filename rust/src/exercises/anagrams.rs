...
pub fn anagrams_for<'a>(word: &'a str, possible_anagrams: &[&'a str]) -> HashSet<&'a str> {
  ...
  let mut word_vec: Vec<String> = word_lower.split("").map(String::from).collect();

  word_vec.sort_unstable();

  for maybe in possible_anagrams {
    let maybe_lower = maybe.to_lowercase();
    let mut maybe_vec: Vec<String> = maybe_lower.split("").map(String::from).collect();

    maybe_vec.sort_unstable();

    if word_vec == maybe_vec && word_lower != maybe_lower {
      set.insert(maybe);
    }
  }
